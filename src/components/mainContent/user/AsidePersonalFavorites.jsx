...
const AsidePersonalFavorites = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`/api/favorites/${titleFavotites.is}/${userInfo.email}`)
      .then(({ data: arrFavorites }) => {
  ...
  const handleSwitchFavorites = (e) => {
    ...
    if (name) {
      ...
      if (titleFavotites.title == "Tus Películas Favoritas") {
        setTimeout(() => {
          setTitleFavorites({
            title: "Tus TvShows Favoritos",
            switcher: "Tus Películas Favoritas",
            is: "tv",
          });
        }, 200);
      } else {
  ...
  return (
    <div
      onClick={handleClosePersonalAside}
      className="asidePersonalFavorites__main"
    >
      <motion.aside 
       initial={{x: '100vw'}}
        ...
        <ul>
          {favorites?.map((fav) => (
            <ListOfFavorite is={titleFavotites.is} key={fav.id} fav={fav} />
          ))}
        </ul>
      </motion.aside>
    </div>
...
const ListOfFavorite = ({ fav, is }) => {
  ...
  const BACKGROUND_URL = backdrop_path
      ...
      <figure>
        ...
        <figcaption>
          ...
          <h2
            style={
              lengthTitle >= 4
                ? { fontSize: "1.2rem", lineHeight: "2rem" }
                : { fontSize: "1.9rem" }
            }
          >
            {fav[titleOrName]}
